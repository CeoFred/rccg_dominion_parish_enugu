...
if(
  ...
  // !empty($_REQUEST['current_location']) &&
  // !empty($_REQUEST['has_arrived'])
  ...
  $courier->departure_date = trim($_REQUEST['departure_date']);
    ...
    $courier->has_arrived = trim($_REQUEST['has_arrived']);
    $courier->current_location = trim($_REQUEST['current_location']);
...
try {
  ...
  // tell the user
  echo $courier->notFound(json_encode($_REQUEST));
